...
import { deepcopy } from 'lib/modules/esm-tools/helpers.mjs';
import type {
  CommandEntry,
  CommandsCollection,
  DebugOptions,
...
import { collectionToObject } from 'src/utils/helpers';
import { createLogger, verbose } from 'src/utils/logging';
import { collectionPathSym, namePathSym } from 'src/utils/private-symbols';
...
const commandCollection = (collectionPath: string[] = []): CommandsCollection => {
...
export class AdaClient extends Client {
  ...
  public findCommand(interaction: InteractionOfCommand): Optional<BasicCallable> {
    ...
    if (subLevelOne?.type < ApplicationCommandOptionType.String) {
      namePath.push(subLevelOne.name);
      if (subLevelOne.type === ApplicationCommandOptionType.SubcommandGroup) {
